  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={onClose}>
      ...
      <div className="relative w-full max-w-[560px] max-h-[90vh] overflow-y-auto rounded-[20px] border"
        style={{ background: 'var(--bg1)', borderColor: 'var(--card-border)' }}
        ...
        <div className="relative px-6 pt-6 pb-4" style={{ background: 'linear-gradient(135deg, rgba(255,92,40,0.08), rgba(139,92,246,0.08))' }}>
          <button onClick={onClose} className="absolute top-4 right-4 w-8 h-8 rounded-full flex items-center justify-center text-[14px] text-[var(--text3)] hover:text-[var(--text)] transition-colors"
            style={{ background: 'var(--bg2)', border: '1px solid var(--card-border)' }}>✕</button>
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-[18px] font-bold text-[var(--text)]">Портрет потребителя</h2>
              <p className="text-[12px] text-[var(--text3)] mt-0.5">Типичный гость вашего ресторана</p>
            </div>
          </div>
        </div>
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          {/* Demographics */}
          <div>
            <h3 className="text-[13px] font-semibold text-[var(--text2)] mb-3 flex items-center gap-2">
            ...
            <div className="grid grid-cols-2 gap-2.5">
              {traits.map(t => (
                <div key={t.label} className="rounded-[10px] px-3 py-2.5 border" style={{ background: 'var(--bg2)', borderColor: 'var(--card-border)' }}>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-[14px]">{t.icon}</span>
                    <span className="text-[10px] text-[var(--text3)] uppercase tracking-wider">{t.label}</span>
                  </div>
                  <span className="text-[13px] font-semibold text-[var(--text)]">{t.value}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2.5">
              {behavior.map(b => (
                <div key={b.label} className="rounded-[10px] px-3 py-2.5 border" style={{ background: 'var(--bg2)', borderColor: 'var(--card-border)' }}>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-[14px]">{b.icon}</span>
                    <span className="text-[10px] text-[var(--text3)] uppercase tracking-wider">{b.label}</span>
                  </div>
                  <span className="text-[13px] font-semibold text-[var(--text)]">{b.value}</span>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-2">
              {topDishes.map((d, i) => (
                <div key={d.name} className="flex items-center gap-3">
                  <span className="text-[11px] font-bold w-4 text-center" style={{ color: i === 0 ? 'var(--accent)' : 'var(--text3)' }}>{i + 1}</span>
                  <div className="flex-1">
                    <div className="flex justify-between mb-1">
                      <span className="text-[12px] text-[var(--text)]">{d.name}</span>
                      <span className="text-[11px] font-bold text-[var(--text2)]">{d.pct}%</span>
                    </div>
                    <div className="rounded-full overflow-hidden" style={{ height: 5, background: 'var(--bg3)' }}>
                      <div className="h-full rounded-full" style={{
                        width: `${d.pct}%`,
                        background: i === 0 ? 'linear-gradient(90deg, var(--accent), #ff8c42)' : 'rgba(255,92,40,0.35)',
                      }} />
                    </div>
                  </div>
                </div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              {motivations.map(m => (
                <div key={m.text} className="flex-1 rounded-[10px] p-2.5 text-center border" style={{ background: 'var(--bg2)', borderColor: 'var(--card-border)' }}>
                  <div className="text-[20px] mb-1">{m.emoji}</div>
                  <div className="text-[16px] font-bold text-[var(--text)]">{m.pct}%</div>
                  <div className="text-[9px] text-[var(--text3)] mt-0.5 leading-tight">{m.text}</div>
                </div>
              ))}
            </div>
          </div>
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div className="rounded-[14px] border p-4 mb-4" style={{ borderColor: 'var(--card-border)', background: 'var(--bg2)' }}>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-[14px] font-semibold text-[var(--text)]">Просмотры за 14 дней</h3>
              <span className="text-[11px] text-[var(--text3)]">всего: 2 847</span>
            </div>
                    ...
                    <div className="w-full rounded-t-[4px] transition-all duration-300 group-hover:brightness-125"
                      style={{
                        ...
                        background: isWeekend
                          ? 'linear-gradient(180deg, #ff8c42, var(--accent))'
            ...
            <div className="flex gap-[3px] mt-1">
              {DAILY_VIEWS.map((v, i) => (
                <div key={i} className="flex-1 text-center text-[8px] text-[var(--text3)]">{v.day}</div>
              ))}
            </div>
            <div className="flex items-center gap-4 mt-2.5 pt-2" style={{ borderTop: '1px solid var(--card-border)' }}>
              <div className="flex items-center gap-1.5">
                <div className="w-2.5 h-2.5 rounded-[2px]" style={{ background: 'linear-gradient(180deg, #ff8c42, var(--accent))' }} />
                <span className="text-[10px] text-[var(--text3)]">Выходные</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="w-2.5 h-2.5 rounded-[2px]" style={{ background: 'rgba(255,92,40,0.4)' }} />
                <span className="text-[10px] text-[var(--text3)]">Будни</span>
              </div>
            </div>
          </div>
          ...
          {/* Popular hours */}
          <div className="rounded-[14px] border p-4 mb-4" style={{ borderColor: 'var(--card-border)', background: 'var(--bg2)' }}>
            <h3 className="text-[14px] font-semibold text-[var(--text)] mb-3">Когда вас ищут</h3>
            <div className="space-y-2">
              {PEAK_HOURS.map(h => (
                <div key={h.time} className="flex items-center gap-3">
                  <span className="text-[11px] text-[var(--text3)] w-12 shrink-0">{h.time}</span>
                  <div className="flex-1 rounded-full overflow-hidden" style={{ height: 8, background: 'var(--bg3)' }}>
                    <div className="h-full rounded-full transition-all" style={{
                      width: `${h.pct}%`,
                      background: h.pct > 60
                        ? 'linear-gradient(90deg, var(--accent), #ff8c42)'
                        : h.pct > 30
                          ? 'linear-gradient(90deg, rgba(255,92,40,0.5), rgba(255,140,66,0.5))'
                          : 'rgba(255,92,40,0.25)',
                    }} />
                  </div>
                  <span className="text-[11px] font-semibold text-[var(--text)] w-8 text-right">{h.pct}%</span>
                </div>
              ))}
            </div>
        ...
        <div style={{ width: 280, minWidth: 280, flexShrink: 0, position: 'sticky', top: 100 }}>
          {/* Consumer Portrait */}
          <div className="rounded-[16px] border p-5 mb-4" style={{ background: 'linear-gradient(180deg, rgba(139,92,246,0.05), var(--bg2))', borderColor: 'rgba(139,92,246,0.15)' }}>
            <div className="flex items-center gap-2.5 mb-4">
              <span className="w-8 h-8 rounded-[10px] flex items-center justify-center text-[16px]"
                style={{ background: 'linear-gradient(135deg, #8b5cf6, var(--accent))' }}>
                🧑
              </span>
              <h3 className="text-[15px] font-bold text-[var(--text)]">Портрет гостя</h3>
            </div>
            ...
            <div className="space-y-2">
              {[
                ...
                <div key={t.label} className="flex items-center gap-2.5 py-1.5" style={{ borderBottom: '1px solid var(--card-border)' }}>
                  <span className="text-[13px] shrink-0">{t.icon}</span>
                  <span className="text-[10px] text-[var(--text3)] flex-1">{t.label}</span>
                  <span className="text-[11px] font-semibold text-[var(--text)]">{t.value}</span>
                </div>
              ))}
            </div>

            <div className="mt-3">
              <div className="text-[10px] text-[var(--text3)] uppercase tracking-wider mb-2">Зачем приходят</div>
              <div className="flex flex-wrap gap-1.5">
                {[
                  ...
                  <div key={m.text} className="rounded-[8px] px-2 py-1.5 text-center" style={{ background: 'var(--bg3)', flex: '1 1 45%' }}>
                    <div className="text-[13px]">{m.emoji}</div>
                    <div className="text-[11px] font-bold text-[var(--text)]">{m.pct}%</div>
                    <div className="text-[8px] text-[var(--text3)]">{m.text}</div>
                  </div>
                ))}
              </div>
            </div>
