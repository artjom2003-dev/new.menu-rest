...
export function ContactsPanel({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  async function loadAll() {
    ...
    try {
      ...
      toast('Не удалось загрузить контакты', 'error');
    } finally {
      setLoading(false);
    }
  }
  ...
  const handleAccept = async (id: number) => {
    ...
    try {
      ...
      refreshCount();
      toast(`${record?.user?.name || 'Пользователь'} добавлен(а) в контакты`, 'success');
    } catch (e: any) {
      const msg = e?.response?.data?.message || 'Ошибка принятия заявки';
      console.error('Accept error:', e?.response?.status, msg);
      toast(msg, 'error');
      loadAll();
    } finally {
      ...
      <div style={{ width: 400, maxWidth: 'calc(100vw - 16px)', maxHeight: 'calc(100vh - 80px)', borderRadius: 20, overflow: 'hidden', background: 'var(--bg)', border: '1px solid var(--card-border)', boxShadow: '0 16px 48px rgba(0,0,0,0.3)', display: 'flex', flexDirection: 'column', animation: 'contactsIn 0.2s ease-out' }}>
        ...
        <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--card-border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            <button onClick={loadAll} title="Обновить" type="button"
            ...
            <button onClick={onClose} type="button" style={{ width: 28, height: 28, borderRadius: 8, border: 'none', background: 'var(--bg3)', cursor: 'pointer', color: 'var(--text3)', fontSize: 13, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '8px 10px' }}>
          {loading && (
            <div style={{ padding: 24, textAlign: 'center', color: 'var(--text3)', fontSize: 12 }}>Загрузка...</div>
          )}
                  ...
                  {searching ? (
                    ...
                    <div key={u.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', borderRadius: 12, marginBottom: 2 }}>
                      ...
                      {invitedIds.has(u.id) ? (
                        ...
                        <button type="button" onClick={() => handleInvite(u.id)} disabled={busyIds.has(u.id)}
                          style={{ padding: '6px 14px', borderRadius: 10, border: 'none', background: 'var(--accent)', color: '#fff', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', opacity: busyIds.has(u.id) ? 0.5 : 1 }}>
                          {busyIds.has(u.id) ? '...' : 'Добавить'}
                        </button>
                      )}
                    </div>
                  ))}
                ...
                <div style={{ marginBottom: 12 }}>
                  ...
                  {pending.map(r => (
                    <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', borderRadius: 12, background: 'rgba(34,197,94,0.04)', border: '1px solid rgba(34,197,94,0.15)', marginBottom: 4 }}>
                      ...
                      <button type="button" onClick={() => handleAccept(r.id)} disabled={busyIds.has(r.id)}
                        style={{ padding: '5px 14px', borderRadius: 8, border: 'none', background: 'rgba(34,197,94,0.15)', color: '#22c55e', fontSize: 12, fontWeight: 600, cursor: busyIds.has(r.id) ? 'wait' : 'pointer', fontFamily: 'inherit', opacity: busyIds.has(r.id) ? 0.5 : 1 }}>
                        {busyIds.has(r.id) ? '...' : 'Принять'}
                      </button>
                      <button type="button" onClick={() => handleDecline(r.id)} disabled={busyIds.has(r.id)}
              ...
              <div>
                ...
                {companions.length === 0 ? (
                  ...
                  <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', borderRadius: 12, marginBottom: 2, transition: 'background 0.15s' }}
                    ...
                    <button type="button" onClick={() => startChat(r.user.id)} title="Написать"
                    ...
                    <button type="button" onClick={() => handleRemove(r.id)} title="Удалить"
